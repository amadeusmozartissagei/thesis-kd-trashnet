#!/usr/bin/env python3
"""
Kaggle-ready Focus-RCNet baseline CE experiment.

This script isolates the protocol change requested for the replication:
    - epochs: 200 instead of 100
    - batch size: 16 instead of 8

The dataset, seed, augmentation, architecture, input size, optimizer, and
loss match Results/notebook2-phase1-focus-rcnet.ipynb. Knowledge distillation
...
Expected Kaggle inputs:
    ...
    Optional teacher checkpoint used only to reuse the exact split indices:
        /kaggle/input/notebooks/hamzapratama/notebook1-teacher-training/
        efficientnet_b4_teacher_best.pth

If the teacher checkpoint is not attached, the script recreates the same
stratified 70/30 split with seed 42.
"""
...
class Config:
    EXPERIMENT_NAME = "Focus-RCNet Baseline CE - 200 Epochs, Batch Size 16"
    EXPERIMENT_ID = "focus-rcnet-baseline-ce-e200-bs16"
    ...
    CHECKPOINT_FILENAME = "focus_rcnet_baseline_ce_e200_bs16_best.pth"
    HISTORY_FILENAME = "training_history_focus_rcnet_baseline_ce_e200_bs16.csv"
    SUMMARY_FILENAME = "summary_focus_rcnet_baseline_ce_e200_bs16.csv"
    CURVES_FILENAME = "training_curves_focus_rcnet_baseline_ce_e200_bs16.png"
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "config": {
            ...
            "use_amp": cfg.USE_AMP,
            "training_mode": "cross_entropy_only",
...
def save_summary(best_val_acc, best_epoch, total_time, output_dir: Path):
    ...
    with path.open("w", newline="") as file:
        ...
        writer.writerow(["epochs", cfg.EPOCHS])
...
def save_curves(history, output_dir: Path):
    ...
    axes[0].set_title("Focus-RCNet Baseline CE Loss")
    ...
    axes[1].set_title("Focus-RCNet Baseline CE Accuracy")
...
def main():
    ...
    # Reset before model initialization to preserve Notebook 2 reproducibility.
    seed_everything(cfg.SEED)
    model = create_focus_rcnet(cfg.NUM_CLASSES).to(device)
    print("\nStarting baseline CE training...", flush=True)
